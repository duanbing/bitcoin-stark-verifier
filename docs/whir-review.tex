...
\date{\small Subject: \texttt{AppliedPQC/bitcoin-stark-verifier}
  ...
  \quad$\cdot$\quad 67 tests passing
  \quad$\cdot$\quad D1 / D2 / D3 fixed, query constrained}
...
\begin{itemize}[leftmargin=1.2em]
  ...
  answers are batched into the next target, and the closing identity checks the accumulated sum.
  What remains is bookkeeping, not arithmetic.
...
\subsection*{The closing identity checks the accumulated sum}
...
\]
So $\hat w'$ is exactly a list of $(\text{weight},\text{point})$ pairs and checking it means
evaluating the final polynomial at each point and taking the weighted sum. \textbf{This is why no
$\eqp$ primitive is needed anywhere} --- $\eqp$ survives only inside something nothing touches until
it becomes an evaluation. An earlier draft of this report assumed otherwise; reading the
accumulation through shows it is not so, which takes the remaining work from ``build a primitive''
down to bookkeeping.
